...
async fn only_tasks_with_an_explicit_date_ever_surface() {
    ...
    // Dated tasks surface whether or not they are ticked: a finished row is
    // the evidence the list is moving, and taking it away on tick is what made
    // completing one feel like it never happened. Undated tasks on a note that
    // is not a day page still do not, which is the separate rule.
    let due = repo::derived::due_on(db, "2026-09-10").await.unwrap();
    assert_eq!(due.len(), 2, "both dated tasks, done or not");
    assert!(due[0].task.text.contains("con fecha"));
    assert!(!due[0].task.done);
    assert!(due[1].task.text.contains("hecha"));
    assert!(due[1].task.done, "ticked, and still on the day");
    assert!(
        due.iter().all(|t| !t.task.text.contains("sin fecha")),
        "an undated task on an ordinary note belongs to no day"
    );
...
async fn a_task_with_no_date_is_findable_instead_of_lost() {
    ...
    let all = repo::derived::all_tasks(db).await.unwrap();
    assert_eq!(all.len(), 2);
    ...
    // Ticking it in the body marks it on both, because the body is the truth.
    // It stays on the list: what changes is how it reads, not whether it is
    // there.
    let done = body.replace("\"checked\":false", "\"checked\":true");
    repo::notes::save_day_page(db, "2026-09-09", &done, "<p/>", "tareas")
        .await
        .unwrap();
    let ticked = repo::derived::all_tasks(db).await.unwrap();
    assert_eq!(ticked.len(), 2);
    assert!(ticked.iter().all(|t| t.task.done));
    assert!(repo::notes::get(db, &page.id).await.unwrap().is_some());
...
async fn a_task_crosses_the_wire_as_flat_fields() {
    ...
    let tasks = repo::derived::all_tasks(db).await.unwrap();
...
async fn a_day_shows_its_own_undated_tasks_but_they_do_not_follow_you() {
    ...
    assert_eq!(repo::derived::all_tasks(db).await.unwrap().len(), 3);
...
async fn ticking_a_task_edits_the_note_and_costs_no_recall() {
    ...
    // It stays on the list, marked, and the first is untouched. Dropping it
    // took the evidence away at the one moment it was worth having.
    let after = repo::derived::all_tasks(db).await.unwrap();
    assert_eq!(after.len(), 2);
    assert!(after.iter().find(|t| t.task.text == "segunda").unwrap().task.done);
    assert!(!after.iter().find(|t| t.task.text == "primera").unwrap().task.done);
    ...
    let same = repo::derived::all_tasks(db).await.unwrap();
    assert!(
        same.iter().find(|t| t.task.id == second.task.id).unwrap().task.done,
        "ticked one is marked, not removed"
    );
    repo::derived::set_done(db, &second.task.id, false).await.unwrap();
    let reopened = repo::derived::all_tasks(db).await.unwrap();
    assert_eq!(reopened.len(), 2);
    assert!(reopened.iter().all(|t| !t.task.done), "and it goes back to open");
