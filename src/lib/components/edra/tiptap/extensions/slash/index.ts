...
import { commands, dueCommands, inTask, type EdraCommand } from '../../../commands/index.js';
			...
			return [
				Suggestion({
					...
					items: ({ query }: { query: string }) => {
						...
						// Dates only where they would mean something. On a paragraph
						// a due date indexes nothing, so offering it there is a lie.
						const groups = inTask(editor)
							? [{ name: 'due', title: 'Due', actions: dueCommands }, ...GROUPS]
							: GROUPS;
